...
export const rejectStoreRegistrationMutation = procedures.adminCc
	.input(
		z.object({
			...
			reason: z
				...
				.min(1, {
					error: () =>
						m.admin_cc_stores_validation_reject_reason_required(
							{},
	...
	.mutation(async ({ ctx, input }) => {
		...
		await writeAuditEventBestEffort({
			context: 'trpc.adminCcStores.rejectRegistration',
		...
		return { registrationRequest: result.reviewedRequest };
	});
