...
const suspendStoreFormSchema = z.object({
	reason: z.string().trim().min(1, {
		error: m.admin_cc_stores_validation_suspend_reason_required(),
	}),
...
const toSuspendStoreSubmitData = (
	...
	const result = suspendStoreFormCodec.safeDecode(formState);
...
const SUSPEND_STORE_FORM_OPTIONS = formOptions({
	defaultValues: { reason: '' } satisfies SuspendStoreFormState,
	validators: { onSubmit: suspendStoreFormCodec },
});
