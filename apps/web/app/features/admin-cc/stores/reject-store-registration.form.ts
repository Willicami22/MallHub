...
const rejectStoreRegistrationFormSchema = z.object({
	reason: z.string().trim().min(1, {
		error: m.admin_cc_stores_validation_reject_reason_required(),
	}),
}) satisfies z.ZodType<{ reason: string }>;

const rejectStoreRegistrationFormCodec = z.codec(
	rejectStoreRegistrationFormState,
	rejectStoreRegistrationFormSchema,
	{
		decode: (formState) => ({ reason: formState.reason.trim() }),
		encode: (formData) => ({ reason: formData.reason }),
	},
);
...
const toRejectStoreRegistrationSubmitData = (
	...
	const result = rejectStoreRegistrationFormCodec.safeDecode(formState);
...
const REJECT_STORE_REGISTRATION_FORM_OPTIONS = formOptions({
	defaultValues: { reason: '' } satisfies RejectStoreRegistrationFormState,
	validators: { onSubmit: rejectStoreRegistrationFormCodec },
});
