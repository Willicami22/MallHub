...
### MallHub guidance: avoid giant TSX components

For MallHub code, this section is mandatory in practice:

- Keep form UI split into smaller components (field groups, section blocks, action footers).
- Prefer local splitting in the same file for small pieces to keep ownership clear and avoid over-fragmenting files.
- Keep state colocated with the UI that mutates it. Avoid central "mega-components" that own all field and UI state.
- For form-related UI state, prefer `formControls` in TanStack Form over ad-hoc `useState` or `useEffect` where possible.
- For direct input updates, use `field.handleChange(...)`; use `form.setFieldValue(...)` only for imperative transitions (async lifecycle, cross-field sync, reset flows).
- Use narrow subscriptions (`form.Field`, focused `form.Subscribe`) to minimize re-renders.
